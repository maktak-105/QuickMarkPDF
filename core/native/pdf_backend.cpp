...
RenderedPage PdfBackend::render_page_at_dpi(const std::string& path, std::size_t page_index, int dpi,
                                             int rotation, const std::string& password,
                                             const std::optional<ClipRectPt>& clip) {
    ...
    auto to_px = [&](double points, int axis_extent) {
        return std::clamp(static_cast<int>(std::lround(points * scale)), 0, axis_extent);
    };
    int x0 = to_px(clip->x0, full.width);
    int x1 = to_px(clip->x1, full.width);
    int y0 = to_px(clip->y0, full.height);
    int y1 = to_px(clip->y1, full.height);
    if (x1 < x0) std::swap(x0, x1);
    if (y1 < y0) std::swap(y0, y1);
    const int crop_w = std::max(1, x1 - x0);
    const int crop_h = std::max(1, y1 - y0);

    RenderedPage cropped;
    cropped.width = crop_w;
    cropped.height = crop_h;
    cropped.rgba.resize(static_cast<std::size_t>(crop_w) * static_cast<std::size_t>(crop_h) * 4);
    for (int y = 0; y < crop_h; ++y) {
        const unsigned char* src_row =
            full.rgba.data() + (static_cast<std::size_t>(y0 + y) * static_cast<std::size_t>(full.width) +
                                 static_cast<std::size_t>(x0)) * 4;
        unsigned char* dst_row = cropped.rgba.data() + static_cast<std::size_t>(y) * static_cast<std::size_t>(crop_w) * 4;
        std::memcpy(dst_row, src_row, static_cast<std::size_t>(crop_w) * 4);
    }
    return cropped;
}
