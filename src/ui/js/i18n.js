...
(() => {
  const STRINGS = {
    ja: {
      ...
      'help.basic.preview': '<strong>プレビュー操作</strong>: マウスホイール（またはCtrl+ホイール）で拡大縮小、右ドラッグでパン移動ができます。右ドラッグでのパン操作後はメニューを表示せず、移動のない短い右クリック時のみメニューが開きます。',
      ...
      'help.markdown.body': '.md ファイルを開くと、PDF編集の代わりにMarkdownプレビューが表示されます。見出し・リスト・表・コードブロックに加えて、Mermaid記法の図（<code>```mermaid</code>）と、$記法（<code>$...$</code> / <code>$$...$$</code>）による数式（MathJax）に対応しています。2MB以上の大容量ファイルは先頭5,000行を瞬時に表示し、下部の「続きを読み込む」ボタンから順次追加表示できます。設定メニューはこの間も利用できます。',
    ...
    en: {
      ...
      'help.basic.preview': '<strong>Preview controls</strong>: Zoom with the mouse wheel (or Ctrl+wheel) and pan by right-dragging. Panning does not open the context menu upon release; the menu opens only on short, stationary right-clicks.',
      ...
      'help.markdown.body': 'Opening a .md file shows a Markdown preview instead of PDF editing. In addition to headings, lists, tables, and code blocks, it supports Mermaid diagrams (<code>```mermaid</code>) and math (MathJax) via <code>$...$</code> / <code>$$...$$</code> notation. Large files (>=2MB) display the first 5,000 lines instantly, with a "Load more" button at the bottom to load subsequent lines. The Settings menu remains available while in this mode.',
